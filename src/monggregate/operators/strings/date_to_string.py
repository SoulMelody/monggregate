...
class DateToString(StringOperator):
    ...
    def expression(self) -> Expression:
        return self.express({"$dateToString": self.dict(by_alias=True, exclude_none=True)})


def date_to_string(
    date: Any, format_: Any = None, timezone: Any = None, on_null: Any = None
) -> DateToString:
    """Returns an $dateToString operator"""

    return DateToString(date=date, format_=format_, timezone=timezone, on_null=on_null)
