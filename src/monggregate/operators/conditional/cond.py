...
class Cond(ConditionalOperator):
    ...
    def _validate_conditional(cls, values: dict) -> dict:
        ...
        if not (c1 or c2):
            raise ValueError(
                "Either (if_, then_, else_) or (expression, true_, false_) must be provided"
            )
    ...
    def expression(self) -> Expression:
        return self.express({"$cond": {"if": self.if_, "then": self.then_, "else": self.else_}})
...
def cond(*args: Any, **kwargs: Any) -> Cond:
    ...
    return Cond(if_=if_, then_=then_, else_=else_)
