...
class Facet(SearchCollector):
    ...
    def init_autocomplete(
        ...
        **kwargs: Any,
        ...
        _autocomplete = Autocomplete(
            query=query, path=path, token_order=token_order, fuzzy=fuzzy, score=score, **kwargs
        )
    ...
    def init_regex(
        ...
        **kwargs: Any,
        ...
        _regex = Regex(
            query=query, path=path, allow_analyzed_field=allow_analyzed_field, score=score
        )
    ...
    def init_text(
        ...
        **kwargs: Any,
        ...
        _text = Text(query=query, path=path, score=score, fuzzy=fuzzy, synonyms=synonyms)
        ...
        **kwargs: Any,
        ...
        if not self.operator:
            ...
            self.operator.autocomplete(
                type=clause_type, minimum_should_match=minimum_should_match, **_autocomplete.dict()
            )
        else:
            new_operator = Compound(
                should=[self.operator, _autocomplete], minimum_should_match=minimum_should_match
            )
        ...
        **kwargs: Any,
        ...
        if not self.operator:
            ...
            self.operator.equals(
                type=clause_type, minimum_should_match=minimum_should_match, **_equals.dict()
            )
        else:
            new_operator = Compound(
                should=[self.operator, _equals], minimum_should_match=minimum_should_match
            )
            self.operator = new_operator
    ...
    def exists(self, path: str, **kwargs: Any) -> Self:
        ...
        if not self.operator:
            ...
            self.operator.exists(
                type=clause_type, minimum_should_match=minimum_should_match, **_exists.dict()
            )
        else:
            new_operator = Compound(
                should=[self.operator, _exists], minimum_should_match=minimum_should_match
            )
            self.operator = new_operator

        return self

    def more_like_this(self, like: dict | list[dict], **kwargs: Any) -> Self:
        ...
        if not self.operator:
            ...
            new_operator = Compound(
                should=[self.operator, _more_like_this], minimum_should_match=minimum_should_match
            )
            self.operator = new_operator
    ...
    def range(
        ...
        **kwargs: Any,
        ...
        if not self.operator:
            ...
            self.operator.range(
                type=clause_type, minimum_should_match=minimum_should_match, **_range.dict()
            )
        else:
            new_operator = Compound(
                should=[self.operator, _range], minimum_should_match=minimum_should_match
            )
            self.operator = new_operator
    ...
    def regex(
        ...
        **kwargs: Any,
        ...
        if not self.operator:
            ...
            self.operator.regex(
                type=clause_type, minimum_should_match=minimum_should_match, **_regex.dict()
            )
        else:
            new_operator = Compound(
                should=[self.operator, _regex], minimum_should_match=minimum_should_match
            )
            self.operator = new_operator
    ...
    def text(
        ...
        **kwargs: Any,
        ...
        if not self.operator:
            ...
            self.operator.text(
                type=clause_type, minimum_should_match=minimum_should_match, **_text.dict()
            )
        else:
            new_operator = Compound(
                should=[self.operator, _text], minimum_should_match=minimum_should_match
            )
            self.operator = new_operator
        ...
        **kwargs: Any,
        ...
        if not self.operator:
            ...
            self.operator.wildcard(
                type=clause_type, minimum_should_match=minimum_should_match, **_wildcard.dict()
            )
        else:
            new_operator = Compound(
                should=[self.operator, _wildcard], minimum_should_match=minimum_should_match
            )
            self.operator = new_operator
        ...
        **kwargs: Any,  # NOTE : To prevent errors from passing extra argumentscf #100 on GitHub <VM, 22/01/2024>
        ...
        if type == "string":
            if num_buckets is None:
                num_buckets = 10
            facet = StringFacet(name=name, path=path, num_buckets=num_buckets)
        elif type == "number":
            facet = NumericFacet(name=name, path=path, boundaries=boundaries, default=default)
        elif type == "date":
            facet = DateFacet(name=name, path=path, boundaries=boundaries, default=default)
        else:
            raise ValueError(
                f"Invalid facet type. Valid facet types are 'string', 'number' and 'date'. Got {type} instead."
            )
    ...
    def numeric(
        ...
        *,
        ...
        self.facet(type="number", path=path, name=name, boundaries=boundaries, default=default)
        return self
    ...
    def date(
        ...
        *,
        ...
        self.facet(type="date", path=path, name=name, boundaries=boundaries, default=default)
        return self

    def string(self, path: str, *, num_buckets: int = 10, name: str | None = None) -> Self:
        """Adds a string facet to the current facet instance."""

        self.facet(type="string", path=path, name=name, num_buckets=num_buckets)
        return self
    ...
    def NumericFacet(
        *,
        ...
        return NumericFacet(name=name, path=path, boundaries=boundaries, default=default)

    @staticmethod
    def StringFacet(
        *, path: str, name: FacetName | None = None, num_buckets: int = 10
    ) -> StringFacet:
        """Returns a string facet instance."""

        return StringFacet(name=name, path=path, num_buckets=num_buckets)
    ...
    def DateFacet(
        *,
        ...
        return DateFacet(name=name, path=path, boundaries=boundaries, default=default)
    ...
    def Facet(
        *,
        ...
        if type == "string":
            facet = Facet.StringFacet(name=name, path=path, num_buckets=num_buckets)
        elif type == "number":
            facet = Facet.NumericFacet(name=name, path=path, boundaries=boundaries, default=default)
        else:
            facet = Facet.DateFacet(name=name, path=path, boundaries=boundaries, default=default)
