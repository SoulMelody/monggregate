...
from monggregate.search.operators.compound import ClauseType, Compound
...
class Pipeline(BaseModel):  # pylint: disable=too-many-public-methods
    ...
    def bucket(
        ...
        *,
        ...
        self.stages.append(
            Bucket(by=by or group_by, boundaries=boundaries, default=default, output=output)
        )
        return self
    ...
    def bucket_auto(
        ...
        *,
        ...
        self.stages.append(
            BucketAuto(by=by or group_by, buckets=buckets, output=output, granularity=granularity)
        )
        return self
    ...
    def __join_common(
        ...
        self.stages.append(
            Lookup(right=right, on=on, left_on=left_on, right_on=right_on, name=join_field)
        )
        self.stages.append(Unwind(path_to_array=join_field))
        self.stages.append(
            ReplaceRoot(document=MergeObjects(operand=[ROOT, "$" + join_field]).expression)
        )
    ...
    def out(
        self, collection: str | None = None, coll: str | None = None, *, db: str | None = None
    ) -> Self:
        """
    ...
    def project(
        ...
        **kwargs: Any,
        ...
        self.stages.append(
            Project(include=include, exclude=exclude, fields=fields, projection=projection)
        )
        return self
    ...
    def replace_root(
        ...
        *,
        ...
        self.stages.append(ReplaceRoot(path=path or path_to_new_root, document=document))
        return self
    ...
    def sort(
        ...
        **kwargs: Any,
        ...
        query = query | kwargs
        self.stages.append(Sort(descending=descending, ascending=ascending, by=by, query=query))
        return self
