...
class Lookup(Stage):
    ...
    on: str | None = None #  shortcut for when left_on is the same than right_on
    ...
    let: (
        dict | None
    ) = None  # the let variables can be accessed by the stages in the pipeline including additional $lookup stages
    # nested in
